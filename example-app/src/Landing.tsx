...
export function Landing() {
  ...
  const pageProgress = useSpring(scrollYProgress, { stiffness: 160, damping: 28, mass: .3 })
  const heroLift = useTransform(scrollYProgress, [0, .2], [0, reduced ? 0 : -46])
  async function copy() {
  ...
  return <MotionConfig reducedMotion="user"><div className="lp">
    <motion.div className="lp-scroll-progress" style={{ scaleX: pageProgress }} />
    <section className="lp-hero lp-frame">
      <div className="lp-hero-rails" aria-hidden="true">{[0, 1, 2, 3].map(i => <motion.i key={i} initial={{ scaleX: 0, opacity: 0 }} animate={{ scaleX: 1, opacity: 1 }} transition={{ duration: .8, delay: .18 + i * .09, ease: [0.22, 1, 0.36, 1] }} />)}</div>
      <a className="lp-announcement" href={`${repository}/releases`}><span />Built in the open. Drift 0.1 alpha <ArrowUpRight size={13} /></a>
      <motion.div style={{ y: heroLift }}><motion.h1 initial={{ opacity: 0, y: 18, filter: 'blur(8px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} transition={{ duration: .7, ease: [0.22, 1, 0.36, 1] }}>Drift.<br /><span>Your next frontend.</span></motion.h1>
      ...
      <span className="lp-copy-status" role="status">{copyError ? 'Select the command above to copy it.' : copied ? 'Copied to clipboard' : 'Open source. React compatible. Yours to build with.'}</span></motion.div>
      <div className="lp-workbench" id="playground">
        ...
        <div className="lp-workbench-body"><div className="lp-editor">
          ...
          <div className="lp-preview-stage"><span className="lp-measure lp-measure-top">a little less ceremony</span><div className="lp-signal-path" aria-hidden="true">{[0,1,2,3].map(i=><motion.i key={i} animate={reduced?{}:{offsetDistance:['0%','100%'],opacity:[0,1,1,0]}} transition={{duration:2.8,repeat:Infinity,delay:i*.55,ease:'linear'}}/>)}</div><motion.button key={replay} initial={{ opacity: 0, y: reduced ? 0 : 12 }} animate={{ opacity: 1, y: 0 }} whileHover={reduced ? undefined : { y: -4, scale: 1.015 }} transition={{ type: 'spring', stiffness: 420, damping: 28 }} whileTap={reduced ? undefined : { scale: .97 }} className="lp-demo-button" onClick={() => setPressed(!pressed)}>{pressed ? <Check size={17} /> : <Sparkles size={17} />}{pressed ? 'Something great, started.' : 'Make something great'}<ArrowRight size={16} /></motion.button><span className="lp-measure lp-measure-bottom">a little more possibility</span><motion.span className="lp-cursor" animate={reduced?{}:{x:[0,-8,0],y:[0,-6,0]}} transition={{duration:3,repeat:Infinity}}><MousePointer2 size={22}/></motion.span></div>
    ...
    <section className="lp-section lp-frame"><div className="lp-section-heading"><span className="lp-eyebrow">01 / MADE FOR THE FRONTEND</span><h2>Everything belongs<br /><span>right where you build.</span></h2><p>Keep the details of an interface together.<br />Let Drift turn them into familiar web output.</p></div>
      <div className="lp-features">
        <motion.article whileHover={reduced?{}:{y:-8}} transition={{type:'spring',stiffness:320,damping:25}}><div className="lp-feature-visual lp-style-visual"><motion.div animate={reduced ? {} : { y: [0, -7, 0], rotate:[-8,-5,-8] }} transition={{ duration: 4, repeat: Infinity }}><span /><span /><span /><span /></motion.div><code>style {'{'} scope: yours {'}'}</code></div><span className="lp-feature-label"><Layers3 size={16} />Style with intent.</span><p>Scoped CSS, shared design tokens, and responsive styles. Bring Tailwind when you need it.</p><Link to="/ui">Explore styling <ArrowUpRight size={14} /></Link></motion.article>
        <motion.article whileHover={reduced?{}:{y:-8}} transition={{type:'spring',stiffness:320,damping:25}}><div className="lp-feature-visual lp-motion-visual"><motion.span animate={reduced ? {} : { x: [-45, 45, -45], rotate: [0, 90, 0], scale:[1,1.12,1] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }}><Wand2 size={22} /></motion.span><span className="lp-motion-track" /><code>hover {'{'} type: spring {'}'}</code></div><span className="lp-feature-label"><Sparkles size={16} />Give it a little life.</span><p>Describe movement alongside your component. Build hover, press, and entrance states into the experience.</p><Link to="/language">Meet the language <ArrowUpRight size={14} /></Link></motion.article>
        <motion.article whileHover={reduced?{}:{y:-8}} transition={{type:'spring',stiffness:320,damping:25}}><div className="lp-feature-visual lp-route-visual">{['pages/index.drift', 'pages/about.drift', 'pages/[slug].drift'].map((route, i) => <motion.div key={route} animate={reduced ? {} : { x:[0,i===1?8:3,0],opacity: [.5, 1, .5] }} transition={{ duration: 3.4, repeat: Infinity, delay: i * .55 }}><Route size={13} /><code>{route}</code><ArrowUpRight size={12} /></motion.div>)}</div><span className="lp-feature-label"><Route size={16} />Go from page to product.</span><p>File-based routes, loaders, nested layouts, and route metadata give your frontend room to grow.</p><Link to="/seo">Explore route metadata <ArrowUpRight size={14} /></Link></motion.article>
      </div>
    </section>
    ...
    <section className="lp-close lp-frame"><div className="lp-close-mark" aria-hidden="true"><motion.i animate={reduced?{}:{x:[-15,15,-15]}} transition={{duration:3.8,repeat:Infinity,ease:'easeInOut'}}/><motion.i animate={reduced?{}:{x:[10,-10,10]}} transition={{duration:4.6,repeat:Infinity,ease:'easeInOut'}}/><b>D</b></div><span className="lp-eyebrow">SMALL BEGINNINGS. BEAUTIFUL INTERFACES.</span><h2>Make your next<br />idea <span>real.</span></h2><div className="lp-actions"><Link to="/language" className="lp-primary">Build with Drift <ArrowRight size={16} /></Link><a href={`${repository}/blob/main/CONTRIBUTING.md`} className="lp-secondary">Help shape Drift <Github size={16} /></a></div><p>Public alpha. Built for curious developers.<br />APIs are evolving, and your feedback matters.</p></section>
